...
This note is an updated version of the final project submission for the numerics class by Aleks Donev in Fall of 2021. This version incorporates some comments by Aleks Donev on the original submitted version. There is no major revisions and some mistakes were acknowledged but uncorrected. This note should act as a sample of final project for this class, instead of a polished note for the public.

\section{Introduction}
We want to solve the linear non-diffusive advection of a passive tracer\footnote{It is possible to add diffusion in the \sml\;framework, see \cite{SpiegelmanKatz_06}.}. Assume $\nabla\cdot\ve u = 0$, we have the Partial Differential Equation (PDE):
\begin{align}
