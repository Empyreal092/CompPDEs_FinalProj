...
We verify the lesson we learn from Section \ref{sec:interp1D} in 2D. We solve again the constant velocity advection problem with $[0,1)\times [0,1)$ with $\ve u = (1,1)$. We again solve to $T = 1$ and compare the solution to the initial condition. We use a 2D trigonometric polynomial as initial condition:
\begin{align*}
    c(x,y,0) &= [\sin(\pi*x)\sin(\pi*y)]^p \hspace{1cm}\text{with }p = 2.
\end{align*}
We show the resultant convergence order estimate in Fig. \ref{fig:2D_const_adv_all_interp}
\begin{figure}[H]
    \centering
